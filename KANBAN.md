...
- [ ] Run `npm run seed` a SECOND time — the earliest run that can observe a closure; until then no close time exists anywhere
...
- [ ] Confirm on a real iOS device that focusing a filter no longer zooms — headless cannot show this **← the only item left**
- [x] Merge to main
